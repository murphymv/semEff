...
-   `getX()` threw an error when trying to construct model design matrices using factors with less than two levels (where contrasts can't be applied). Such an error might occur where e.g. the dataset called by a model contained at least one *unspecified* character/factor variable with only one unique value, which was incorrectly coerced to a factor by `getX()` (typically in order to gain access to dummy variables not in the original design matrix).
